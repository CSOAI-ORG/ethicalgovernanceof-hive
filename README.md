...
```
L7  PRESENTATION    ethics indigo + care pink (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      meok-governance-engine-mcp, care-membrane-mcp, ai-bom-mcp, explainability-report-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: ethical frameworks, value alignment, AI BOM, explainability
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "ethicalgovernanceof"
...
```bash
git clone https://github.com/CSOAI-ORG/ethicalgovernanceof-hive
cd ethicalgovernanceof-hive
...
`https://ethicalgovernanceof.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
Redirect to csoai.org (don't dilute brand).
